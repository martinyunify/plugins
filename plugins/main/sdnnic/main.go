...
type PluginConf struct {
	...
	RuntimeConfig *struct {
		PluginConfig map[string]interface{} `json:"plugin"`
	} `json:"runtimeConfig"`
...
func parseConfig(stdin []byte) (*PluginConf, error) {
	...
	if conf.PluginName == "" {
		return nil, fmt.Errorf("PluginName must be specified")
	}

	if _, exist := PluginRegistry[conf.PluginName]; !exist {
		return nil, fmt.Errorf("Plguin is not supported")
	}
	return &conf, nil
}
